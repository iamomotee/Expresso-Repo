...
st.markdown("<h1 style = 'color: #0C2D57; text-align: center; font-family: helvetica'>CHURNS EXPRESSO AVAILABLE</h1>", unsafe_allow_html = True)
...
st.sidebar.image('username.png', caption = 'Welcome User')
...
tenure = st.sidebar.selectbox('TENURE', ds['TENURE'].unique())
montant = st.sidebar.number_input('MONTANT', ds['MONTANT'].min(), ds['MONTANT'].max())
freq_rech = st.sidebar.number_input('FREQUENCE_RECH', ds['FREQUENCE_RECH'].min(), ds['FREQUENCE_RECH'].max())
rev = st.sidebar.number_input('REVENUE', ds['REVENUE'].min(), ds['REVENUE'].max())
arpu = st.sidebar.number_input('ARPU_SEGMENT', ds['ARPU_SEGMENT'].min(), ds['ARPU_SEGMENT'].max())
data_vol = st.sidebar.number_input('DATA_VOLUME', ds['DATA_VOLUME'].min(), ds['DATA_VOLUME'].max())
onnet = st.sidebar.number_input('ON_NET', ds['ON_NET'].min(), ds['ON_NET'].max())
